...
def tactical_profile(request):
    ...
    if created:
        logger.info(
            f"PROFILE_RECOVERY: Generated missing identity record for Agent {user.username}."
        )
...
def tactical_profile_edit(request):
    ...
    if request.method == "POST":
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            profile_form.save()
            logger.info(
                f"RECORD_SYNC: Agent {user.username} synchronized identity parameters."
            )
            messages.success(request, "SERVICE RECORD UPDATED.")
            return redirect("accounts:profile")
        else:
            logger.warning(
                f"RECORD_SYNC_FAIL: Validation errors for Agent {user.username}."
            )
