...
def tactical_password_change(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            logger.info(
                f"AUTH_SECURITY: Agent {user.username} updated credentials successfully."
            )
            messages.success(request, "PASSWORD CHANGED SUCCESSFULLY.")
            return redirect("accounts:profile")
        else:
            logger.warning(
                f"AUTH_SECURITY_FAIL: Credential update rejected for Agent {request.user.username}."
            )
...
def tactical_login(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            login(request, user)
            logger.info(
                f"ACCESS_GRANTED: Agent {user.username} established secure link."
            )
