...
class TankLookupTests(TestCase):
    def setUp(self):
        ...
        self.store_other = Store.objects.create(
            store_num=1234, store_name="Other Store", city="Other City", state="OC"
        )
        ...
        StoreTankMapping.objects.create(
            store=self.store_std, tank_type=self.tank_type_reg, fuel_type="regular"
        )

        # Create Mappings for Other Store
        StoreTankMapping.objects.create(
            store=self.store_other, tank_type=self.tank_type_dsl, fuel_type="diesel"
        )
