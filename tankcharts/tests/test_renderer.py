...
class PDFRendererTests(TestCase):
    ...
    def test_render_store_returns_multi_page_pdf(self):
        ...
        tank_type = TankType.objects.create(
            name="12k120", capacity=12000, max_depth=120
        )

        for tank_index, fuel_name in [(1, "regular"), (2, "diesel")]:
            mapping = StoreTankMapping.objects.create(
                store=store,
                tank_type=tank_type,
                fuel_type=fuel_name,
        ...
        store_chart = service.build_store(store_num=7001)
        chunks = service.chunk_store_tanks(store_chart, page_size=4)
        pdf_bytes = PDFRenderer().render_store(store_chart, tank_chunks=chunks)
