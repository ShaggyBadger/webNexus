...
def validate_store(request):
    ...
    if request.method == "GET":
        ...
        if not query:
            return JsonResponse({"valid": False, "message": "Query parameter is required."}, status=400)
        
        try:
            val = int(query.strip())
            store = Store.objects.filter(Q(store_num=val) | Q(riso_num=val)).first()
            if store:
                logger.info(f"STORE_VALIDATE_SUCCESS: Found Store {store.store_num} (RISO: {store.riso_num}) for query: '{query}'.")
                return JsonResponse({
                    "valid": True,
                    "id": store.id,
                ...
                logger.info(f"STORE_VALIDATE_NOT_FOUND: No match in DB for store query: '{query}'.")
                return JsonResponse({"valid": False})
        except ValueError:
            logger.info(f"STORE_VALIDATE_INVALID: Query '{query}' is not a valid integer.")
            return JsonResponse({"valid": False, "message": "Store number must be an integer."})
