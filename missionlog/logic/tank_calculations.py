...
def calculate_gallons(store_identifier, fuel_type_name, inches):
    ...
    try:
        ...
        if not mapping:
            logger.debug(f"CALC_FAIL: No tank mapping found for store {store.store_num} with fuel type '{fuel_type_name}'.")
            return None
        
        tank_type = mapping.tank_type
        charts = TankChart.objects.filter(tank_type=tank_type).order_by("inches")
        if not charts.exists():
            logger.debug(f"CALC_FAIL: No calibration charts found for TankType '{tank_type.name}' (Store: {store.store_num}).")
            return None
        ...
        logger.debug(f"CALC_SUCCESS: Interpolated {inches_float}\" in {tank_type.name} to {interpolated:.2f} gal.")
