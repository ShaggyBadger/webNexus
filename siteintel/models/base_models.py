...
class LocationType(models.Model):
    ...
    description = models.TextField(
        blank=True, null=True, help_text="Details regarding site characteristics"
    )
...
class SiteAttributeDefinition(models.Model):
    ...
    label = models.CharField(
        max_length=100, help_text="Human-readable label (e.g., 'Vapor Manifold')"
    )
    ...
    is_required = models.BooleanField(
        default=False, help_text="If true, this field is mandatory in proposal forms"
    )
...
class Location(models.Model):
    ...
    tactical_overlay = models.TextField(
        blank=True, null=True, help_text="Canonical tactical drawings (GeoJSON)"
    )
