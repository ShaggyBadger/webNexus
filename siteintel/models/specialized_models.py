...
class Yard(models.Model):
    ...
    notes = models.TextField(
        blank=True, null=True, help_text="Yard-specific operational details"
    )
...
class FuelRack(models.Model):
    ...
    config_json = models.JSONField(
        default=dict, blank=True, help_text="Structured Lane/Arm configuration"
    )
...
class RackCheckIn(models.Model):
    ...
    rack = models.ForeignKey(
        FuelRack, on_delete=models.CASCADE, related_name="check_ins"
    )
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="rack_check_ins"
    )
    ...
    is_verified = models.BooleanField(
        default=False, help_text="True if captured within proximity of the rack."
    )
