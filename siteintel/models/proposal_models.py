...
class StoreUpdate(models.Model):
    ...
    submitted_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="submitted_updates"
    )
    ...
    proposed_metadata = models.JSONField(
        default=dict, blank=True, help_text="Proposed changes to site quirks"
    )

    # Specialized Data Proposals
    rack_lockout_days = models.IntegerField(
        null=True, blank=True, help_text="Proposed lockout period for Fuel Rack"
    )
    rack_config_json = models.JSONField(
        default=dict,
        blank=True,
        help_text="Proposed Lane/Arm configuration for Fuel Rack",
    )
    yard_notes = models.TextField(
        null=True, blank=True, help_text="Proposed yard-specific operational details"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            if old_status != self.status:
                logger.info(
                    f"StoreUpdate ID {self.id} status changed from {old_status} to {self.status} by {self.approved_by or 'System'}"
                )
...
class TankUpdate(models.Model):
    ...
    store_update = models.ForeignKey(
        StoreUpdate, on_delete=models.CASCADE, related_name="tank_updates"
    )
    ...
    reported_capacity = models.IntegerField(
    ...
    def __str__(self):
        return (
            f"Tank {self.tank_index}: {self.fuel_type} ({self.reported_capacity} gal)"
        )
...
class MapOverlayUpdate(models.Model):
    ...
    location = models.ForeignKey(
        "siteintel.Location", on_delete=models.CASCADE, related_name="overlay_updates"
    )
    ...
    submitted_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="submitted_overlays"
    )
    submitted_at = models.DateTimeField(auto_now_add=True)
    ...
    def apply_overlay(self):
        ...
        with transaction.atomic():
            ...
            self.location.save()
            logger.info(
                f"OVERLAY_SYNC: Applied GeoJSON for Location {self.location.id}"
            )
