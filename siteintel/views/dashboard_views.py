...
class FuelRackListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        """
        logger.info(
            f"RACK_SECTOR_ACCESS: Fuel Rack list accessed by Agent {self.request.user}"
        )
        ...
        for rack in context["racks"]:
            ...
            rack_data.append({"rack": rack, "status": status})
...
class SiteIntelDashboardView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if q:
            logger.info(
                f"DASHBOARD_QUERY: Filtering by '{q}' triggered by {self.request.user}"
            )
