...
def initialize_location_for_store(request, store_id):
    ...
    if not store.location:
        logger.info(
            f"INTEL_BOOTSTRAP: Initializing Location record for Store #{store.store_num}"
        )
        ...
        store.save()
        logger.info(
            f"INTEL_INIT_SUCCESS: Store #{store.store_num} linked to Location ID {loc.id} by {request.user}"
        )
