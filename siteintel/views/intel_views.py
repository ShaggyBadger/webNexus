...
class LocationDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if context["fuel_rack"]:
            context["rack_status"] = (
                rack_ops.get_rack_status(self.request.user, context["fuel_rack"])
        ...
        personal_intel = None
        default_intel = SiteIntelligence.objects.filter(
        ...
        logger.info(
            f"METADATA_LOAD: Processed {len(ordered_attrs)} attributes for Location {loc.id}"
        )
        ...
        logger.info(
            f"SITE_ACCESS: Site ID {loc.id} accessed by {self.request.user.username if self.request.user.is_authenticated else 'ANONYMOUS'}"
        )
        return context
...
class MapOverlayUpdateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["location"] = get_object_or_404(
            Location, id=self.kwargs.get("location_id")
        )
        return context
    ...
    def form_valid(self, form):
        ...
        logger.info(
            f"OVERLAY_PROPOSAL: Map update submitted for Site {loc.id} by Agent {self.request.user}"
        )
    ...
    def get_success_url(self):
        ...
        return reverse(
            "siteintel:location_detail", kwargs={"pk": self.kwargs.get("location_id")}
        )
...
class SiteIntelligenceUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_object(self, queryset=None):
        ...
        intel, created = SiteIntelligence.objects.get_or_create(
            location=location, author=self.request.user, defaults={"notes": ""}
        )
        if created:
            logger.info(
                f"INTEL_INIT: Initialized new field report for Site {location_id} by Agent {self.request.user}"
            )
        return intel
    ...
    def get_success_url(self):
        ...
        return reverse(
            "siteintel:location_detail", kwargs={"pk": self.object.location.id}
        )
