...
def proximity_check_api(request):
    ...
    try:
        ...
        logger.warning(
            f"PROXIMITY_CHECK_FAILED: Invalid coordinates provided: {lat}, {lon}"
        )
...
def rack_status_api(request):
    ...
    if rack_id:
        ...
        return JsonResponse(
            {"rack_id": rack.id, "name": rack.location.name, "status": status}
        )
...
def tank_type_search_api(request):
    ...
    matches = TankType.objects.filter(filters).values(
        "id", "name", "manufacturer", "capacity", "max_depth"
    )[:20]
...
def store_lookup_api(request):
    ...
    store = Store.objects.filter(Q(store_num=query) | Q(riso_num=query)).first()
...
def site_lookup_api(request):
    ...
    logger.info(
        f"TARGET_SCAN_COMPLETE: Identified {len(results)} potential targets for ID '{q}'"
    )
