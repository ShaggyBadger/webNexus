...
class StoreUpdateFormsetMixin:
    ...
    def get_context_data(self, **kwargs):
        ...
        existing_types = (
            Store.objects.exclude(store_type__isnull=True)
            .exclude(store_type="")
        ...
        if self.request.POST:
            ...
            if self.object:
                ...
                if initial_tanks:
                    logger.info(
                        f"TANK_SYNC: Initializing {len(initial_tanks)} tank forms for Store {store.store_num if store else 'UNK'}"
                    )
                    CustomFormSet = forms.inlineformset_factory(
                        StoreUpdate,
                        TankUpdate,
                        form=TankUpdateForm,
                        extra=len(initial_tanks),
                        can_delete=True,
                    )
        ...
        if tank_formset.is_valid():
            ...
            for tank in tanks:
                if tank.tank_index is None:
                    while any(
                        t.tank_index == next_auto_index
                        for t in tanks
                        if t.tank_index is not None
                    ):
                        next_auto_index += 1
            ...
            logger.info(
                f"PROPOSAL_ACTION: Success for Store {store_num} by {self.request.user}"
            )
...
class StoreUpdateUpdateView(
    LoginRequiredMixin, UserPassesTestMixin, StoreUpdateFormsetMixin, UpdateView
):
    """
    OPERATIONAL FLOW:
...
class StoreUpdateListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return StoreUpdate.objects.filter(submitted_by=self.request.user).order_by(
            "-submitted_at"
        )
