...
def apply_proposal(proposal):
    ...
    if proposal.status != "APPROVED":
        logger.warning(
            f"SYNC_HALTED: Attempted to apply unapproved Update ID {proposal.id}"
        )
        raise ValueError("Only approved updates can be applied to canonical data.")

    logger.info(
        f"SYNC_START: Processing Update ID {proposal.id} (Type: {proposal.location_type})"
    )
    ...
    with transaction.atomic():
        ...
        if not location_type:
            ...
            if created:
                logger.info(
                    f"SYNC_CLASSIFICATION: Created fallback LocationType '{loc_type_name}'"
                )
        ...
        if not proposal.location:
            ...
            logger.info(
                f"SYNC_GEO: Bootstrapped new Location record ID {proposal.location.id}"
            )
        else:
            ...
            if proposal.proposed_metadata is not None:
                ...
                logger.info(
                    f"SYNC_METADATA: Synchronized {len(proposal.proposed_metadata)} attributes for Location ID {proposal.location.id}"
                )

            proposal.location.save()
            logger.info(
                f"SYNC_GEO: Updated geospatial foundation for Location ID {proposal.location.id}"
            )
...
def _sync_store(proposal):
    ...
    if not proposal.store:
        ...
        if existing_store:
            ...
            logger.info(
                f"SYNC_STEP: Linked Update {proposal.id} to existing Store #{proposal.store.store_num}"
            )
        else:
...
def _sync_fuel_rack(proposal):
    ...
    if not created:
        ...
        rack.save()
        logger.info(
            f"SYNC_STEP: Updated FuelRack record for Location ID {proposal.location.id}"
        )
    else:
        logger.info(
            f"SYNC_STEP: Created new FuelRack record for Location ID {proposal.location.id}"
        )
...
def _sync_yard(proposal):
    ...
    yard, created = Yard.objects.get_or_create(
        location=proposal.location, defaults={"notes": proposal.yard_notes or ""}
    )
    ...
    if not created:
        ...
        yard.save()
        logger.info(
            f"SYNC_STEP: Updated Yard record for Location ID {proposal.location.id}"
        )
    else:
        logger.info(
            f"SYNC_STEP: Created new Yard record for Location ID {proposal.location.id}"
        )
