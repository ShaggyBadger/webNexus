...
def get_store_local_today(store):
    ...
    if location and location.timezone:
        try:
            ...
            logger.error(
                f"UST_SERVICE_TZ_ERROR: Failed to get local time for store {store.store_num}: {str(e)}"
            )
...
def confirm_permit(store, user, notes=None):
    ...
    return USTVerification.objects.create(
        store=store, user=user, verification_type="confirmed", notes=notes
    )
...
def update_permit(store, user, permit_data, notes=None):
    ...
    if "expiration_date" in permit_data:
        permit_data["expiration_date"] = normalize_expiration_date(
            permit_data["expiration_date"]
        )
    ...
    new_permit = USTPermit.objects.create(store=store, is_active=True, **permit_data)

    # 3. Create verification record
    verification = USTVerification.objects.create(
        store=store, user=user, verification_type="updated", notes=notes
    )

    logger.info(
        f"UST_SERVICE: Updated permit for store {store.store_num} by {user.username}"
    )
