...
const TacticalGPS = {
    ...
    async updateUI(coords) {
        ...
        try {
            const response = await fetch(`/tankgauge/api/closest-store/?lat=${coords.latitude}&lon=${coords.longitude}`);
            if (response.ok) {
                const data = await response.json();
                intelDisplay.style.display = 'block';
                intelDisplay.innerHTML = `
                    ZONE: ${data.city.toUpperCase()}, ${data.state.toUpperCase()} // 
                    TARGET: #${data.store_num} (${data.distance_feet.toLocaleString()} FT)
                `;
            }
        } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => TacticalGPS.init());
